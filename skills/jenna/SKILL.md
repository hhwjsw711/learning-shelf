---
name: learning-shelf
description: Publish and maintain Jenna's learning doc on The Shelf — a shared directory of living HTML learning docs. Use when asked to create, update, publish, or view a learning doc/log on the shelf, or to start documenting a new topic Jenna is learning.
---

# The Shelf — Jenna's contributor skill
...
- **Your human is**: `Jenna` — always publish with `author=jenna`.
- **Your band design is**: `daisy-days` — always publish with `authorStyle=daisy-days`.
...
2. **Pick a template for each doc before your first contribution — mandatory.**
   ...
   - Read `~/beautiful-html-templates/index.json`, shortlist 2–3 templates whose
     mood fits Jenna's topic, and ask Jenna to pick one.
...
4. **Keep the local source file** in Jenna's project or home directory. The
...
6. **Read and iterate against the HOSTED copy** at `https://noah-learning-shelf.vercel.app/d/<slug>`, not the
   local file, so Jenna sees exactly what everyone else sees.
...
- `author` and `authorStyle` are fixed for Jenna (above); `template` is per doc.
...
- Verify after: `curl -s "https://noah-learning-shelf.vercel.app/d/<slug>" | head -5` shows your doctype, and the
  directory at `https://noah-learning-shelf.vercel.app/` lists it under Jenna's band with a fresh date.
