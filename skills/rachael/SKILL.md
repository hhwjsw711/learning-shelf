---
name: learning-shelf
description: Publish and maintain Rachael's learning doc on The Shelf — a shared directory of living HTML learning docs. Use when asked to create, update, publish, or view a learning doc/log on the shelf, or to start documenting a new topic Rachael is learning.
---

# The Shelf — Rachael's contributor skill
...
- **Your human is**: `Rachael` — always publish with `author=rachael`.
- **Your band design is**: `block-frame` — always publish with `authorStyle=block-frame`.
...
2. **Pick a template for each doc before your first contribution — mandatory.**
   ...
   - Read `~/beautiful-html-templates/index.json`, shortlist 2–3 templates whose
     mood fits Rachael's topic, and ask Rachael to pick one.
...
4. **Keep the local source file** in Rachael's project or home directory. The
...
6. **Read and iterate against the HOSTED copy** at `https://noah-learning-shelf.vercel.app/d/<slug>`, not the
   local file, so Rachael sees exactly what everyone else sees.
...
- Verify after: `curl -s "https://noah-learning-shelf.vercel.app/d/<slug>" | head -5` shows your doctype, and the
  directory at `https://noah-learning-shelf.vercel.app/` lists it under Rachael's band with a fresh date.
