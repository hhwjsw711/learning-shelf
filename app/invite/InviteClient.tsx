...
export function InviteClient({ viewer }: { viewer: string | null }) {
  ...
  async function mint() {
    ...
    if (!res.ok) {
      ...
      // Keep the kit around: the owner cookie the server just set marks this
      // browser as a member, and next visits show the kit from here instead
      // of the mint form.
      if (data.installer) {
        try {
          localStorage.setItem("shelf-kit", data.installer);
          localStorage.setItem("shelf-kit-author", data.author ?? "");
        } catch {
          // storage may be unavailable (private mode) — the kit still shows
        }
      }
    }
    setBusy(false);
...
function AlreadyOnBoard({ viewer }: { viewer: string }) {
  ...
  useEffect(() => {
    try {
      setKit(localStorage.getItem("shelf-kit"));
    } catch {
      // storage unavailable — fall through to the no-kit copy
    }
  }, []);
